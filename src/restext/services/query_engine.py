...
async def execute_query(
    ...
    # 2. Vector search (over-fetch for re-ranking), with Postgres fallback
    raw_results = []
    try:
        ...
        print(f"[QUERY] Qdrant failed, using Postgres fallback: {type(e).__name__}: {e}", flush=True)
        from sqlalchemy import select, or_
        from restext.models.chunk import Chunk
        keywords = [w for w in request.query.lower().split() if len(w) > 2][:5]
        if keywords:
            conditions = [Chunk.content.ilike(f"%{kw}%") for kw in keywords]
            result = await db.execute(
                select(Chunk).where(Chunk.project_id == project_id, or_(*conditions)).limit(request.top_k * 2)
            )
            for c in result.scalars().all():
                raw_results.append({"id": str(c.id), "score": 0.5, "payload": {"content": c.content}})
